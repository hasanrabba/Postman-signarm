...
import { persistFailure, subscribePersistFailure } from "@/lib/persistStatus";
...
export default function Home() {
  ...
  // A failed save used to be invisible: work stayed on screen and was gone
  // after a reload.
  const saveError = useSyncExternalStore(subscribePersistFailure, persistFailure, () => null);
  ...
  return (
    <div className="flex h-screen bg-signal-bg text-signal-text">
      {saveError && (
        <div
          role="alert"
          className="fixed top-0 inset-x-0 z-50 bg-signal-err text-white text-xs px-3 py-1.5 text-center"
        >
          {saveError}
        </div>
      )}
