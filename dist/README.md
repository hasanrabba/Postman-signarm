...
This folder contains a prebuilt Windows binary for Signarm Signal,
cross-compiled from Linux using the GNU toolchain
...
| File | Size | Purpose |
|---|---|---|
| `SignarmSignal.exe` | ~4.6 MB | The main GUI executable (PE32+, stripped) |
| `WebView2Loader.dll` | 160 KB | Runtime shim that loads the system WebView2 |

Keep both files together — `SignarmSignal.exe` looks for
`WebView2Loader.dll` in its own directory at launch.
...
build host is Linux. A Windows-native MSVC build
(`x86_64-pc-windows-msvc`, the path documented in the main README) will:
